...
(function () {
  ...
  /* The rail stops at the footer — navy label on Reef Petrol would be illegible.
     (visual-direction-02.html §05: "rail extent — it stops at the footer") */
  var rail = document.querySelector('.rail');
  var footer = document.querySelector('.footer');
  if (rail && footer && 'IntersectionObserver' in window) {
    new IntersectionObserver(function (entries) {
      rail.classList.toggle('hide', entries[0].isIntersecting);
    }, { threshold: 0 }).observe(footer);
  }

  /* A <select> always has a value, so its floating label starts raised.
     Textareas and inputs are handled by :not(:placeholder-shown) in CSS. */
  document.querySelectorAll('.fgroup select').forEach(function (s) {
    s.closest('.fgroup').classList.add('sel');
  });
})();
